...
boston_x_train, boston_x_test, boston_y_train, boston_y_test = train_test_split(boston_x, boston_y, test_size=0.3,
                                                                                random_state=42)
result = {i: Ridge(alpha=i / 10).fit(boston_x_train, boston_y_train).score(boston_x_test, boston_y_test) for i in
          range(1, 11, 1)}
print(result)  # 就结果来讲，正则修正系数越小越好，当然这也是在剔除异常值之前的了，以alpha=0为佳
"""
boston_regression = Ridge(alpha=0.5).fit(boston_x_train, boston_y_train)  # 线性回归拟合并评分
print(boston_regression.score(boston_x_test, boston_y_test))
boston_y_pretict = boston_regression.predict(boston_x_test)
picture_boston_Ridge = plt.figure()
subpic_boston_Ridge = picture_boston_Ridge.add_subplot(111)
subpic_boston_Ridge.scatter(boston_y_test, boston_y_pretict)  # 绘制预测实际价格参数
plt.show()
"""
